...
/**
 * Hook quản lý logic cho màn hình Xem chi tiết nhân viên (ADM003).
 * Bao gồm: Lấy dữ liệu từ API và xử lý các sự kiện nút bấm.
 */
export const useADM003 = () => {
  ...
  /**
   * Hiệu ứng lấy dữ liệu chi tiết nhân viên khi ID thay đổi.
   */
  useEffect(() => {
    if (id) {
      setLoading(true);
   ...
   * Hiển thị xác nhận và gọi API xóa (sẽ triển khai sau).
   */
  const handleDelete = async () => {
    if (window.confirm('Bạn có chắc chắn muốn xóa nhân viên này không?')) {
      // TODO: Triển khai gọi API xóa tại đây
